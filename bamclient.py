...
bam_api_pass = parser.get('BlueCat Address Manager', 'bam_api_pass')
bam_config_name = parser.get('BlueCat Address Manager', 'bam_config_name')
...
def GetDeviceUDF(soap_client,name):
	DeviceUDF = soap_client.service.getUserDefinedFields("Device",False)
	if hasattr(DeviceUDF, 'item'):
		for d in DeviceUDF.item:
				if (d.name == name):
					return d.name
	return
...
def AddUDF(soap_client,name,displayname):
	udf_attributes = {
		'type': 'TEXT',
		'defaultValue': '',
		'validatorProperties': '',
	...
	try:
		UDF = soap_client.service.addUserDefinedField('Device', dict(udf_attributes,name=name,displayName=displayname))
...
#Add a Configuration
def AddAWSConfiguration(soap_client,name):
